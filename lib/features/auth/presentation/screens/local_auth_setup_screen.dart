  ...
  Future<void> _checkBiometrics() async {
    try {
      ...
      if (supported) {
        ...
        setState(() {
          _isFingerprintAvailable = available.contains(BiometricType.fingerprint) || available.contains(BiometricType.strong);
                ...
                if (_currentStep == 0) ...[
                      ...
                      child: Wrap(
                        ...
                        children: [
                          if (_isFingerprintAvailable)
                            Chip(
                              avatar: const Icon(Icons.fingerprint, size: 16, color: Colors.teal),
                              label: const Text('Fingerprint Supported', style: TextStyle(color: Colors.teal, fontSize: 12)),
                          ...
                          if (!_isFingerprintAvailable && _biometricsSupported)
                            Chip(
                              ...
                              label: const Text('Biometrics Supported', style: TextStyle(color: Colors.teal, fontSize: 12)),
                              backgroundColor: Colors.teal[50],
                              side: BorderSide.none,
                              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
                            ),
                          if (!_biometricsSupported)
                            Chip(
                              ...
                              label: const Text('No Biometrics Found', style: TextStyle(color: Colors.orange, fontSize: 12)),
                  ...
                  if (_isFingerprintAvailable) ...[
                    SwitchListTile(
                      ...
                      subtitle: const Text('Use registered fingerprints to unlock the app'),
                      value: _enableFingerprint,
                      onChanged: (value) => setState(() => _enableFingerprint = value),
                    ),
                    const Divider(),
                  ],
                  if (!_isFingerprintAvailable && _biometricsSupported) ...[
                    SwitchListTile(
                      ...
                      title: const Text('Enable Fingerprint Unlock',
                          style: TextStyle(fontWeight: FontWeight.bold)),
                      subtitle: const Text('Use local device fingerprint authentication to unlock'),
                      value: _enableFingerprint,
                  ...
                  children: [
                    if (_currentStep > 0)
                      TextButton(
                        ...
                        child: const Text('Back', style: TextStyle(color: Colors.teal, fontSize: 16)),
                      )
                    else
                      const SizedBox(),
